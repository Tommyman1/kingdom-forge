...
app.get("/health", (_request, response) => response.json({ status: "ok", version: "2.1.0", time: new Date().toISOString() }));
...
app.get("/api/admin/diagnostics", requireUser, (request, response) => {
  ...
  response.json({ version: "2.1.0", schema: 2, databaseBytes: fs.statSync(databasePath).size, users: db.prepare("SELECT COUNT(*) count FROM users").get().count, campaigns: db.prepare("SELECT COUNT(*) count FROM campaigns").get().count, events: db.prepare("SELECT COUNT(*) count FROM campaign_events").get().count, journalMode: db.pragma("journal_mode", { simple: true }), dataDir });
});
